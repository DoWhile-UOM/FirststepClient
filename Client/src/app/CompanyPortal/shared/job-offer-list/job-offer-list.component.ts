...
export class JobOfferListComponent implements AfterViewInit {
  ...
  async refreshTable(status: string) {
    ...
    await this.advertisementService
      .getAllAdvertisementsByCompanyID(this.company_id, status)
      .then((response) => {
        ...
        for (let i = 0; i < this.jobList.length; i++) {
          Table_data.push({
            ...
            no_of_evaluated_applications:
              this.jobList[i].no_of_evaluated_applications,
            no_of_accepted_applications:
              this.jobList[i].no_of_accepted_applications,
            no_of_rejected_applications:
              this.jobList[i].no_of_rejected_applications,
          });
        }
  ...
  filter(selected: any) {
    ...
    this.snackBar
      .open(
        'Refeshing table to show ' + selected.value + ' job offers...',
        '',
...
export class ConfirmDialog {
  ...
  async onYesClick() {
    if (this.dialogtitle == 'Close') {
      ...
      this.snackBar
        .open(this.title + ' job offer successfully deleted!', '', {
          panelClass: ['app-notification-normal'],
        })
        ._dismissAfter(5000);
      ...
      this.snackBar
        .open(this.title + ' job offer successfully activate again!', '', {
          panelClass: ['app-notification-normal'],
        })
        ._dismissAfter(5000);
    }
